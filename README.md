...
In the year 2100, passengers no longer navigate fragmented transportation networks. **NOVA** is a Universal Mobility Operating System that unifies autonomous pods, ultra-high-speed HyperRail, sub-orbital/regional AeroLink shuttles, and accessible walking infrastructure into a single, cohesive journey.
...
- **Travel Style Preferences**: Instant switching between *Fastest*, *Calmest*, *Eco*, and *Low walking* (default demo state).
...
- **Accessible Bottom Sheet**: Blurred backdrop with drag pill handle.
- **Quick Locations**: One-tap shortcuts for *Home (Ratmalana District)* and *Work (Colombo Central)*.
- **Recent Destinations**: Includes `Colombo Skyport` with the `✦ Best connection now` coral badge, `Central District`, and `Port City Ocean Hub`.
...
- **Accessibility Guarantee**: Verified 64% walking reduction with elevator confirmation.
...
- **Custom Responsive SVG City Map**: Zero external dependencies. Renders stylized districts, transit grids, travelled lines, active Luciferin Green routes, and Quantum Coral future paths.
...
- **Live Action Sheet**: Immediate action prompt (*"Exit at Central Skyport · Doors open on the left"*), verified AeroLink Gate 04 transfer tag, and 3-column transit metrics.
- **Approaching Transfer State**: Amber-accented gentle reminder when 2 minutes away from Central Skyport.
- **Key Wow Moment — AI Network Change & Rerouting**:
  - Simulates an unexpected 6-minute delay on AeroLink A12.
  - NOVA autonomously recalculates an alternative connection via AeroLink Express A14.
  - Passenger accepts with one tap: the signature path animation morphs the trajectory, maintaining protected arrival at **09:42**.
...
- **Font Scale**: No text below 12px; headings in Space Grotesk; body in Inter.
...
- **Mobile First**: Pixel-perfect on standard mobile screens (375px–430px) with elegant centered desktop responsiveness up to 1100px.
